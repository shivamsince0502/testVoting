...
async function extractPackageFile(_content, packageFile, _config) {
    ...
    for (const { name, path } of depNames) {
        try {
            ...
            let httpSubModuleUrl = (0, url_1.getHttpUrl)(subModuleUrl);
            const hostType = (0, common_1.detectPlatform)(httpSubModuleUrl) ?? git_refs_1.GitRefsDatasource.id;
            httpSubModuleUrl = (0, url_1.getRemoteUrlWithToken)(httpSubModuleUrl, hostType);
            const currentValue = await getBranch(gitModulesPath, name, httpSubModuleUrl);
            deps.push({
                depName: path,
                packageName: (0, url_1.getHttpUrl)(subModuleUrl),
                currentValue,
                currentDigest,
            });
        }
