...
function extractPackageFile(content, packageFile, extractConfig) {
    ...
    try {
        ...
        if (!config) {
            logger_1.logger.debug({ packageFile }, 'Null config when parsing Docker Compose content');
            return null;
        }
        if (typeof config !== 'object') {
            logger_1.logger.debug({ packageFile, type: typeof config }, 'Unexpected type for Docker Compose content');
            return null;
        }
    }
    catch (err) {
