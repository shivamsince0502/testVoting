...
async function addSourceCmds(packageFileName, _config, nugetConfigFile) {
    ...
    for (const registry of registries) {
        ...
        if (username) {
            ...
            addSourceCmd += ` --username ${(0, shlex_1.quote)(username)}`;
        }
        if (password) {
            // Add password from host rules, if configured.
            addSourceCmd += ` --password ${(0, shlex_1.quote)(password)} --store-password-in-clear-text`;
        }
...
async function runDotnetRestore(packageFileName, dependentPackageFileNames, config) {
    ...
    const nugetConfigFile = (0, upath_1.join)(nugetCacheDir, `nuget.config`);
    await (0, fs_1.ensureDir)(nugetCacheDir);
    await (0, fs_1.outputCacheFile)(nugetConfigFile, `<?xml version="1.0" encoding="utf-8"?>\n<configuration>\n</configuration>\n`);
    const cmds = [
...
async function updateArtifacts({ packageFileName, newPackageFileContent, config, updatedDeps, }) {
    ...
    const deps = await (0, package_tree_1.getDependentPackageFiles)(packageFileName, isCentralManament);
