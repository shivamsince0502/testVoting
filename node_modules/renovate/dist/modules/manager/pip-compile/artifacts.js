...
exports.extractResolver = exports.updateArtifacts = exports.constructPipCompileCmd = void 0;
...
function constructPipCompileCmd(content, inputFileName, outputFileName) {
    ...
    if (headers?.groups) {
        logger_1.logger.debug(`Found pip-compile header: ${headers[0]}`);
...
async function updateArtifacts({ packageFileName: inputFileName, newPackageFileContent: newInputContent, config, }) {
    ...
    try {
        ...
        logger_1.logger.trace({ cmd }, 'pip-compile command');
...
function extractResolver(argument) {
    const value = argument.replace('--resolver=', '');
    if (['backtracking', 'legacy'].includes(value)) {
        return value;
    }
    logger_1.logger.warn({ argument }, 'pip-compile was previously executed with an unexpected `--resolver` value');
    return null;
}
