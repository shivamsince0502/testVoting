...
async function getRepos(config) {
    ...
    const url = 'projects?' + (0, url_1.getQueryString)(queryParams);
    try {
...
async function ignoreApprovals(pr) {
    try {
        ...
        const existingRegularApproverRules = rules?.filter(({ rule_type, name }) => rule_type !== 'any_approver' && name !== ruleName);
    ...
    if (platformOptions?.usePlatformAutomerge) {
        try {
            ...
            for (let attempt = 1; attempt <= retryTimes; attempt += 1) {
                ...
                await (0, promises_1.setTimeout)(500 * attempt);
            }
...
exports.getPr = getPr;
async function updatePr({ number: iid, prTitle, prBody: description, state, platformOptions, targetBranch, }) {
    let title = prTitle;
...
async function setBranchStatus({ branchName, context, description, state: renovateState, url: targetUrl, }) {
    ...
    try {
        ...
        await (0, promises_1.setTimeout)(process.env.RENOVATE_X_GITLAB_BRANCH_STATUS_DELAY
