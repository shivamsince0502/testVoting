...
async function initPlatform({ endpoint, token: originalToken, username, gitAuthor, }) {
    ...
    if (token.startsWith('github_pat_')) {
        throw new Error('Init: Fine-grained Personal Access Tokens do not support the GitHub GraphQL API and cannot be used with Renovate.');
    }
...
async function listForks(token, repository) {
    try {
        ...
        const repos = (await githubApi.getJson(url, {
            token,
            paginate: true,
            pageLimit: 100,
        })).body;
...
async function initRepo({ endpoint, repository, forkOrg, forkToken, renovateUsername, cloneSubmodules, ignorePrAuthor, }) {
    ...
    if (branchExists) {
        try {
            ...
            if (err.err?.response?.statusCode === 422) {
                logger_1.logger.debug({ branchResult, err }, 'Branch update failed due to reference not existing - will try to create');
            }
            else {
                logger_1.logger.warn({ refUrl, err, branchResult }, 'Error updating branch');
                throw err;
            }
        }
    }
...
async function getStatus(branchName, useCache = true) {
    ...
    return (await githubApi.getJson(commitStatusUrl, {
        memCache: useCache,
    })).body;
}
...
async function getStatusCheck(branchName, useCache = true) {
    ...
    return (await githubApi.getJson(url, { memCache: useCache })).body;
}
...
async function getIssue(number, useCache = true) {
    ...
    try {
        const issueBody = (await githubApi.getJson(`repos/${config.parentRepo ?? config.repository}/issues/${number}`, { memCache: useCache })).body.body;
        return {
            number,
            body: issueBody,
        };
...
async function updatePr({ number: prNo, prTitle: title, prBody: rawBody, state, targetBranch, }) {
