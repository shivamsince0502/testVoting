...
async function initRepo(args) {
    ...
    git = (0, simple_git_1.default)(global_1.GlobalConfig.get('localDir'), (0, config_1.simpleGitConfig)()).env({
        ...process.env,
        LANG: 'C.UTF-8',
        LC_ALL: 'C.UTF-8',
    });
...
async function isBranchModified(branchName) {
    ...
    const isModified = (0, modified_cache_1.getCachedModifiedResult)(branchName, getBranchCommit(branchName) // branch sha
    );
    if (isModified !== null) {
...
async function mergeBranch(branchName, localOnly = false) {
    let status;
    try {
        await syncGit();
        await writeGitAuthor();
